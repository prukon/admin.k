...
final class UserCustomPaymentPublicPayService
{
    ...
    /** Без 0/O/1/I/l — проще прочитать в сообщении. */
    ...
    public function ensureFreshLink(UserCustomPayment $payment): UserCustomPaymentPublicPayLink
    {
        $payment->loadMissing('user');
        $partnerId = (int) $payment->partner_id;

        /** @var UserCustomPaymentPublicPayLink $link */
        $link = UserCustomPaymentPublicPayLink::query()->firstOrNew(
            ['user_custom_payment_id' => $payment->id],
    ...
    public function publicShareUrl(UserCustomPaymentPublicPayLink $link): string
    {
        $code = trim((string) ($link->short_code ?? ''));
        if ($code !== '') {
            return route('ucp.public.pay.short', ['code' => $code], true);
        }

        return route('ucp.public.pay', ['token' => $link->token], true);
    }

    /**
     * Смена суммы: отменяет активный T‑Bank-платёж и выпускает новый token + short_code.
     * Старые /pay/ucp/{token} и /pc/{code} перестают открываться.
     */
    public function resetPublicPayAfterAmountChange(UserCustomPayment $payment): ?string
    {
        if ($payment->effective_is_paid) {
            return null;
        }
    ...
    public function cancelActivePublicPay(UserCustomPayment $payment): void
    {
        $link = UserCustomPaymentPublicPayLink::query()
            ->where('user_custom_payment_id', $payment->id)
            ->first();
    ...
    private function generateUniqueShortCode(): string
    {
        ...
        for ($attempt = 0; $attempt < 16; $attempt++) {
            ...
            $taken = UserCustomPaymentPublicPayLink::query()
     ...
     */
    public function syncPublicPayPaymentForQr(UserCustomPaymentPublicPayLink $link, Request $request): array
    {
        ...
        $payment = $link->customPayment()->with(['user:id,partner_id'])->first();
        if (! $payment || (int) ($payment->user?->partner_id ?? 0) !== (int) $link->partner_id) {
        ...
        if (! $this->partnerTbankConfigured((int) $link->partner_id, $teamId > 0 ? $teamId : null)) {
    ...
    public function resolvePublicShow(UserCustomPaymentPublicPayLink $link, Request $request): array
    {
        ...
        $payment = $link->customPayment()->with(['user:id,partner_id', 'team'])->first();
        if (! $payment || (int) ($payment->user?->partner_id ?? 0) !== (int) $link->partner_id) {
            return ['kind' => 'error', 'message' => 'Дополнительный платеж не найден'];
        }
        ...
        $checkoutDisplay = $this->resolvePublicPayCheckoutDisplay((int) $link->partner_id, $teamId);
    ...
    private function ensureActiveTinkoffPaymentId(
        ...
        UserCustomPayment $payment,
        int $studentUserId,
        int $amountCents,
        Request $request,
    ): string {
        $partnerId = (int) $link->partner_id;
        $customPaymentId = (int) $payment->id;
        ...
        $studentUser = $payment->user ?? $payment->user()->first();
        if (! $studentUser) {
            return '__FAIL__';
        }
        ...
        $paymentDate = $note !== '' ? $note : 'Дополнительный платеж';
        ...
        $bankPayment = $this->tinkoffPayments->initPayment($partnerId, $amountCents, 'sbp', [
        ...
        if (empty($bankPayment->tinkoff_payment_id)) {
            return '__FAIL__';
        }

        $intent->tbank_order_id = (string) $bankPayment->order_id;
        $intent->tbank_payment_id = (int) $bankPayment->tinkoff_payment_id;
        $intent->provider_inv_id = (int) $bankPayment->tinkoff_payment_id;
        $intent->save();

        $link->tinkoff_payment_id = (string) $bankPayment->tinkoff_payment_id;
        $link->payment_intent_id = (int) $intent->id;
        $link->payable_id = (int) $payable->id;
        $link->save();

        return (string) $bankPayment->tinkoff_payment_id;
    }

    private function activePaymentAmountMatches(UserCustomPaymentPublicPayLink $link, int $expectedAmountCents): bool
    {
    ...
    private function invalidateActivePublicPayPayment(UserCustomPaymentPublicPayLink $link): void
    {
        $paymentId = (string) ($link->tinkoff_payment_id ?? '');
        if ($paymentId === '') {
            return;
        ...
        if (
            ...
            try {
                ...
                if (! is_array($cancelResponse) || empty($cancelResponse['Success'])) {
                    Log::channel('tinkoff')->warning('[ucp-public-pay cancel] PaymentId='.$paymentId, [
                        'response' => $cancelResponse,
                    ]);
                }
            } catch (\Throwable $e) {
                Log::channel('tinkoff')->warning(
                    '[ucp-public-pay cancel failed] PaymentId='.$paymentId.' '.$e->getMessage()
                );
            }
        }
    ...
    private function markLinkedPublicPayRecordsCancelled(UserCustomPaymentPublicPayLink $link): void
    {
        if ($link->payment_intent_id) {
            PaymentIntent::query()
    ...
    private function clearPublicPayLinkPaymentBinding(UserCustomPaymentPublicPayLink $link): void
    {
        $link->tinkoff_payment_id = null;
        $link->payment_intent_id = null;
        $link->payable_id = null;
    ...
    public function tinkoffQrJson(UserCustomPaymentPublicPayLink $link, string $dataType, Request $request): \Illuminate\Http\JsonResponse
    {
        ...
        $payment = $link->customPayment()->with('user:id,partner_id')->first();
        if (! $payment || $payment->effective_is_paid || (int) ($payment->user?->partner_id ?? 0) !== (int) $link->partner_id) {
    ...
    public function tinkoffQrState(UserCustomPaymentPublicPayLink $link): \Illuminate\Http\JsonResponse
    {
        if ($link->expires_at === null || $link->expires_at->isPast()) {
