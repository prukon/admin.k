...
class DocumentationController extends Controller
{
    ...
    private const PAGE_TITLES = [
        ...
        'dashboard-cabinet'           => 'Консоль (/cabinet): блоки оплат по cabinetPackages.* / cabinetSeasons / customPayments, селект группы фильтрует сезоны и абонементы по team_id, добавление группы в ЛК (account.user.team.update), in-app уведомление admin/trainer (текст с родителем / без), системные мониторы (settings.systemMonitors.view, переключатель в шапке, оверлей Reverb, оверлей онлайн по партнёрам, оверлей Пульт — ховер last_message 500, ViewException, ring errors.recent, Welcome: mailable_class / тема «Доступ в личный кабинет», не точка чата)',
        ...
        'admin-users'                 => 'Клиенты (админка): /admin/users вкладка «Клиенты», только role=user (label «Клиент»), «Показать N» (page_length), телефон родителя в таблице, адрес проживания (users.address → {{child_address}}), ФИО ученика в родительном (users.full_name_genitive → {{child_full_name_genitive}}), импорт Excel, родители, договор (создать / черновик / иконка signed + плюс ещё один, встроенная модалка, lockUser), welcome-письмо (mailable_class в журнале; пульт Welcome — только лид→клиент), пол, комментарий, персональная скидка % (users.discount.manage), team_ids',
        'contracts'                   => 'Договоры (клиентские): PDF и режим «форма клиенту», создание со списка клиентов/заявок (lockUser), блок без юрлица группы, карточка, revoke/refund, вкладка «Шаблоны», колокольчик админам при signed',
        'contract-templates'          => 'Шаблоны DOCX: fields_schema, {{contract_date}} / системные поля, разрывы Word w:t, «Юр. лицо», email, версии',
        'account-contract-fill'       => 'Заполнение договора родителем: fill/generate/sign, каунтер активных договоров в сайдбаре и на вкладке «Мои документы», авто {{contract_date}}, юрлицо группы, паспорт (серия и номер), fallback parent_email, ошибки Laravel под полями, колокольчик админам при signed',
