...
class DocumentationController extends Controller
{
    ...
    private const PAGE_TITLES = [
        ...
        'reports-payments'            => 'Отчёт «Платежи»: payments.team_id (колонка/фильтр), «Поля списка», «Показать N» (page_length), поиск DataTables только ФИО и группа (§1.2), скрытое reports.payments.commission_total.view, история T‑Bank; ФИО = payments.user_id (семья — выбранный ребёнок)',
        'reports-admin'               => 'Отчёты (админка): KidsCrmDataTable, columns-settings, «Показать N» (page_length), поиск DataTables payments/LTV/monthly/intents (§1.2), «Исходящие письма» (модалка, iframe HTML, mailable_class из Mailable / X-Mailable-Class; Laravel 10 сам ключ не ставит; пульт Welcome), фильтры, AJAX/суммы',
        ...
        'setting-prices-monthly-users' => 'Установка цен: бывшие участники, персональная скидка, sync users_prices↔ULP, ручная пролонгация месяца на следующий',
        ...
        'contracts'                   => 'Договоры (клиентские): PDF и режим «форма клиенту», создание со списка клиентов/заявок (lockUser), блок без юрлица группы, ЭП Подпислона с ключа юрлица (не PODPISLON_API_KEY), карточка, revoke/refund, аннулирование sent/opened без возврата 70 ₽ (signed PDF при revoked, без колокольчика), вкладка «Шаблоны», колокольчик админам при signed, колонка «Обновлён» = последнее событие журнала (не contracts.updated_at)',
