...
class DocumentationController extends Controller
{
    ...
    private const PAGE_TITLES = [
        ...
        'schedule-trainer-salary'     => 'ЗП тренеров: схемы classic и kansas, типы тренера, модалка «Настройки месяца», целые средние Канзаса (факт вверх после десятой, база только целое), смена схемы без слепка, *_cents, pivot визита, autosave, слепки',
        ...
        'dashboard-cabinet'           => 'Консоль (/cabinet): блоки оплат по cabinetPackages.* / cabinetSeasons / customPayments, селект группы фильтрует сезоны и абонементы по team_id, добавление группы в ЛК (account.user.team.update), in-app уведомление admin/trainer (текст с родителем / без), оверлей Reverb (settings.reverbOverlay.manage, кнопка на /admin/settings)',
        ...
        'chat' => 'Чат CRM (Сообщения): 1-на-1 и групповые чаты внутри школы, авто-чат учебной группы (threads.team_id), имя группы в списке не «Диалог» (пустое — «Группа», text-align left), бейдж badge-info без вспышки в открытом диалоге, messages.view в базовых ролях, Reverb без wsPath без HTTP-опроса inbox, оверлей статуса процесса/сокета при флаге и settings.reverbOverlay.manage, онлайн (ping без messages.view), подзаголовок шапки (участники / был(а) в сети), галочки исходящего в списке, карточка собеседника из шапки, состав группы из шапки (добавить/удалить — admin/superadmin, покинуть — любой, 0 участников soft-delete кроме team_id), черновик на сервере (превью «Черновик:»), фильтр контактов по учебной группе, создание группы (название + участники), мобильная нижняя панель с планшета (992px): Личные — !is_group, Чаты — #groupThreads is_group без поиска и сплит-бейджи, inbox JSON смешанный без unread_private, шапка кабинета видна, в диалоге низ скрыт, prefetch истории, зум выключен только на /chat, Vite CSS/JS только на /chat, суперадмин в чате как «Служба поддержки», UNIQUE участника (thread_id, user_id), повтор лички после 0/1 живого не плодит тред',
