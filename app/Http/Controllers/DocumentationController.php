...
class DocumentationController extends Controller
{
    ...
    private const PAGE_TITLES = [
        ...
        'partners-permissions'        => 'Партнёры: базовые роли и права; опциональные (customPayments, account.user.team.update, скрытая группа schoolSchedule, …)',
        ...
        'settings-permission-groups'  => 'Матрица прав: группы permissions (permission_groups), аккордеон «Права и роли», группа «Расписание школы» (schoolSchedule), группа «Оплата платформы» (platformPayments.method.*), ховер возможностей права (иконка i)',
        ...
        'reports-admin'               => 'Отчёты (админка): KidsCrmDataTable, columns-settings, «Показать N» (page_length), закрепление thead и горизонтального скролла payments/monthly/LTV/debts (§1.3), поиск DataTables payments/LTV/monthly/intents (§1.2), «Платежи T‑Bank» (Чек / комиссия платформы без additional.value.view), «Исходящие письма» (модалка, iframe HTML, mailable_class из Mailable / X-Mailable-Class; Laravel 10 сам ключ не ставит; пульт Welcome), фильтры, AJAX/суммы',
        ...
        'tbank'                       => 'T‑Bank (мультирасчёты): глобальный терминал, комиссии, автовыплата, карточка платежа (организация, timeline), вкладка отчётов «Платежи T‑Bank» (Чек / комиссия платформы), СБП (QR); публичные /pm и /p без логина — не /tinkoff/qr',
        ...
        'lesson-packages'             => 'Абонементы: шаблоны (UI тип flexible = «Предоплата»), автосписание, заморозка и срок действия (поля при scheduleSlots.view), назначения, короткая ссылка /p/{code} и SMS на 1 сегмент (70 ₽ с баланса; причина ошибки sms.ru в модалке), автопролонгация fixed (новый цикл = текущий % скидки × цена шаблона), billing_month / период, ends_at, конфликт fixed, статусы, Excel',
        ...
        'parents-and-family-cabinet'  => 'Родители и семейный кабинет: parents, parent_id, переключение детей, оплата месяца активного ученика (FamilyPaymentPayerResolver), добавление группы (account.user.team.update), колокольчик attach (текст с родителем), свой пароль в ЛК (тот же → 422 под полем), active_student, Мои документы activeStudent и ссылка из письма ?student=&fill=',
