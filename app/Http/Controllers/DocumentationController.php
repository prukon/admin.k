...
class DocumentationController extends Controller
{
    ...
    private const PAGE_TITLES = [
        ...
        'lesson-packages'             => 'Абонементы: шаблоны, назначения, короткая ссылка /p/{code} и SMS на 1 сегмент (70 ₽ с баланса), автопролонгация fixed (новый цикл = текущий % скидки × цена шаблона), billing_month / период, ends_at, конфликт fixed, статусы, Excel',
        ...
        'parents-and-family-cabinet'  => 'Родители и семейный кабинет: parents, parent_id, переключение детей, оплата месяца активного ученика (FamilyPaymentPayerResolver), добавление группы (account.user.team.update), колокольчик attach (текст с родителем), active_student',
        'dashboard-cabinet'           => 'Консоль (/cabinet): блоки оплат по cabinetPackages.* / cabinetSeasons / customPayments, селект группы фильтрует сезоны и абонементы по team_id, добавление группы в ЛК (account.user.team.update), in-app уведомление admin/trainer (текст с родителем / без)',
        ...
        'chat' => 'Чат CRM (Сообщения): 1-на-1 внутри школы, бейдж badge-info без вспышки в открытом диалоге, messages.view в базовых ролях, Reverb без wsPath, оверлей статуса для superadmin',
