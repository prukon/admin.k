...
class DocumentationController extends Controller
{
    ...
    private const PAGE_TITLES = [
        ...
        'reports-payments'            => 'Отчёт «Платежи»: payments.team_id (колонка/фильтр), «Поля списка», права, история T‑Bank; ФИО = payments.user_id (семья — выбранный ребёнок)',
        'reports-admin'               => 'Отчёты (админка): KidsCrmDataTable, columns-settings, «Исходящие письма» (модалка, iframe HTML), фильтры, AJAX/суммы',
        'reusable-ui-partials'        => 'Переиспользование UI: KidsCrmTooltip / KidsCrmDataTable, toolbar, logModal, Select2 (в т.ч. тренеры журнала)',
        ...
        'schedule-trainer-salary'     => 'ЗП тренеров: схемы classic и kansas, типы тренера (Канзас), смена схемы без слепка, *_cents, pivot визита, autosave, слепки',
        ...
        'admin-users'                 => 'Ученики (админка): /admin/users только role=user, телефон родителя в таблице, адрес проживания (users.address → {{child_address}}), ФИО ученика в родительном (users.full_name_genitive → {{child_full_name_genitive}}), импорт Excel, родители, договор, welcome-письмо, пол, комментарий, персональная скидка % (users.discount.manage), team_ids',
        ...
        'school-leads-widget'         => 'Заявки с сайта: виджет iframe, CRM (порядок колонок ребёнок→родитель→статус→объект→секция→телефон), лид→клиент (welcome-письмо), статус после клиента, договор, статусы, Telegram',
