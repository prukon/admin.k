...
class DocumentationController extends Controller
{
    ...
    private const PAGE_TITLES = [
        ...
        'admin-users'                 => 'Клиенты (админка): /admin/users вкладка «Клиенты», только role=user (label «Клиент»), «Показать N» (page_length), телефон родителя в таблице, адрес проживания (users.address → {{child_address}}), ФИО ученика в родительном (users.full_name_genitive → {{child_full_name_genitive}}), импорт Excel, родители, договор (создать / черновик / иконка signed, встроенная модалка, lockUser), welcome-письмо (mailable_class в журнале; пульт Welcome — только лид→клиент), пол, комментарий, персональная скидка % (users.discount.manage), team_ids',
        'contracts'                   => 'Договоры (клиентские): PDF и режим «форма клиенту», создание со списка клиентов/заявок (lockUser), блок без юрлица группы, карточка, revoke/refund, вкладка «Шаблоны»',
