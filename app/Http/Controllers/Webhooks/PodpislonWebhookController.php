...
class PodpislonWebhookController extends Controller
{
    public function handle(Request $request)
    {
        ...
        try {
            ...
            $fileId   = isset($parsed['FILE_ID'])
                ? (int) $parsed['FILE_ID']
                : (isset($parsed['ID']) ? (int) $parsed['ID'] : null);

            // матчим контракт
            $contract = $fileId
                ? Contract::where('provider_doc_id', $fileId)->first()
                : null;
            ...
            $log->debug('Contract matched', [
                'rid'         => $rid,
                'contract_id' => $contract->id,
            ]);

            // нормализуем тип события для БД
            $etype = match (true) {
                $eventRaw === 'DOCUMENT_SIGNED' || str_contains($eventRaw, 'SIGN')
                    => 'webhook_document_signed',
                $eventRaw === 'DOCUMENT_OPENED' || str_contains($eventRaw, 'OPEN')
                    => 'webhook_document_opened',
                default
                    => 'webhook_' . strtolower($eventRaw ?: 'unknown'),
            };
            ...
            if (in_array($etype, ['webhook_document_signed', 'webhook_document_opened'], true)) {
                ...
                if ($isDup) {
                    $log->warning('Duplicate webhook suppressed', [
                        'rid'   => $rid,
                        'etype' => $etype,
                    ]);
                }
            ...
            if ($etype === 'webhook_document_opened') {
                ...
                DB::transaction(function () use (&$contract, $parsed, $rid, $log) {
                    ...
                    if ($currentStatus !== Contract::STATUS_OPENED) {
                        ...
                        try {
                            $oldLabel = \App\Models\Contract::$STATUS_RU[$currentStatus]
                            ...
                            $log->error('Webhook OPENED: MyLog(contract_opened) FAILED', [
                                ...
                                'trace'       => method_exists($mylogEx, 'getTraceAsString')
                                    ? $mylogEx->getTraceAsString()
                                    : null,
                            ]);
                        }
                    } else {
            ...
            if ($etype === 'webhook_document_signed') {
                ...
                DB::transaction(function () use (&$contract, $parsed, $rid, $log, &$becameSigned) {
                    ...
                    if ($contractForUpdate->status !== Contract::STATUS_SIGNED) {
                        ...
                        try {
                            $oldLabel = \App\Models\Contract::$STATUS_RU[$oldStatus]
                                ?? ucfirst($oldStatus);
