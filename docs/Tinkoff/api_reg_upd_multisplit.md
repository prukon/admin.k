...
Сделано: `POST register`, ручной PATCH с р/с+БИК+банк+назначение, GET (кнопки «Обновить статус» / «Подтянуть из банка»).
...
Не сделано в этом заходе: полный PATCH `bankAccount` перед автовыплатой (сейчас уходит только `details` → 400); отправка `korAccount`; отдельная кнопка `disableReimbursement=false`.
