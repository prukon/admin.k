...
<div class="outgoing-email-show-content">
    @unless($inModal)
    ...
    @else
        <div class="d-flex flex-wrap gap-2 align-items-center mb-3">
            ...
            @if($log->subject)
                <span class="text-muted small text-truncate">{{ $log->subject }}</span>
            @endif
        </div>
    @endunless
    ...
    @php
        $emailBodyTabsId = $inModal ? 'emailBodyTabsModal' : 'emailBodyTabs';
        $emailHtmlTabId = $inModal ? 'email-html-tab-modal' : 'email-html-tab';
        $emailTextTabId = $inModal ? 'email-text-tab-modal' : 'email-text-tab';
        $emailHtmlPaneId = $inModal ? 'email-html-modal' : 'email-html';
        $emailTextPaneId = $inModal ? 'email-text-modal' : 'email-text';
    @endphp

    <ul class="nav nav-tabs" id="{{ $emailBodyTabsId }}" role="tablist">
        <li class="nav-item" role="presentation">
            <button class="nav-link active" id="{{ $emailHtmlTabId }}" data-bs-toggle="tab" data-bs-target="#{{ $emailHtmlPaneId }}" type="button" role="tab">HTML</button>
        </li>
        <li class="nav-item" role="presentation">
            <button class="nav-link" id="{{ $emailTextTabId }}" data-bs-toggle="tab" data-bs-target="#{{ $emailTextPaneId }}" type="button" role="tab">Текст</button>
        </li>
    </ul>
    <div class="tab-content border border-top-0 rounded-bottom p-3 bg-white">
        <div class="tab-pane fade show active" id="{{ $emailHtmlPaneId }}" role="tabpanel">
            @if(! empty($log->html_body))
                <iframe
                    sandbox=""
                    src="data:text/html;charset=utf-8;base64,{{ base64_encode($log->html_body) }}"
                    title="HTML-версия письма"
                    style="width:100%; min-height: 480px; border: 1px solid #e5e5e5; border-radius: 4px;"
                ></iframe>
            @else
                <div class="text-muted">HTML-тело отсутствует.</div>
            @endif
        </div>
        <div class="tab-pane fade" id="{{ $emailTextPaneId }}" role="tabpanel">
