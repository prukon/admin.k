...
@section('content')
    <div class="container">
        <h1>Редактировать платёжную систему</h1>
        ...
        <form action="{{ route('payment-systems.update', $paymentSystem) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <select name="partner_id" id="partner_id" class="form-select">
                    ...
                    @foreach($partners as $partner)
                        <option value="{{ $partner->id }}"
                                {{ old('partner_id', $paymentSystem->partner_id) == $partner->id ? 'selected' : '' }}>
                            {{ $partner->name }}
                        </option>
                    @endforeach
                </select>
