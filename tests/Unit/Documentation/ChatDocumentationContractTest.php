...
final class ChatDocumentationContractTest extends TestCase
{
    ...
    public function test_doc_catalog_and_controller_title_mention_no_flash_and_no_wspath(): void
    {
        ...
        $this->assertStringContainsString('оверлей статуса процесса/сокета при персональном флаге и settings.systemMonitors.view', $controller);
        $this->assertStringContainsString('оверлей онлайн по партнёрам (не точка чата)', $controller);
    ...
    public function test_doc_index_announces_chat_mobile_without_contradicting_live_ux(): void
    {
        ...
        $this->assertNotFalse($start);
        $end = strpos($html, 'id="system-monitors-index"');
        $this->assertNotFalse($end);
        $this->assertGreaterThan($start, $end);
        $chunk = substr($html, $start, $end - $start);
    ...
    public function test_doc_index_announces_reverb_overlay_without_contradicting_live_ux(): void
    {
        ...
        $this->assertStringContainsString('/doc#system-monitors-index', $chunk);
        $this->assertStringContainsString('/doc#online-users-overlay-index', $chunk);
