...
/**
 * Персональное «Показать N» на /admin/school-leads: page_length в user_table_settings.
 */
final class SchoolLeadsPageLengthFeatureTest extends CrmTestCase
{
    ...
    public function test_index_uses_default_page_length_when_nothing_saved(): void
    {
        ...
        $html = $this->get(route('admin.school-leads'))
            ->assertOk()
            ->assertViewHas('leadsPageLength', 10)
            ->getContent();
    ...
    public function test_ajax_save_page_length_persists_without_columns(): void
    {
        ...
        $payload = $this->getJson(route('admin.school-leads.columns-settings.get'))
            ->assertOk()
            ->assertExactJson([])
            ->json();
    ...
    public function test_guest_cannot_save_page_length(): void
    {
        ...
        $response = $this->postJson(route('admin.school-leads.columns-settings.save'), [
            'page_length' => 20,
        ]);
    ...
    public function test_after_changing_show_by_reopening_page_keeps_saved_length(): void
    {
        ...
        $chunk = $this->leadsTableCreateChunk($html);
    ...
    public function test_hiding_columns_then_reopening_page_does_not_reset_show_by(): void
    {
        ...
        $html = $this->get(route('admin.school-leads'))
            ->assertOk()
            ->assertViewHas('leadsPageLength', 100)
            ->getContent();

        $this->assertMatchesRegularExpression('/pageLength:\s*100\b/', $this->leadsTableCreateChunk($html));
    ...
    {
        UserTableSetting::updateOrCreate(
            ['user_id' => $this->foreignUser->id, 'table_key' => 'school_leads_index'],
            ['page_length' => 100]
        );
        ...
        $this->assertMatchesRegularExpression('/pageLength:\s*10\b/', $this->leadsTableCreateChunk($html));
    }

    public function test_landing_and_widget_tabs_do_not_opt_in_to_page_length_persist(): void
    {
        ...
        $this->assertStringNotContainsString('persistPageLength: true', $landing);
        $this->assertStringNotContainsString("KidsCrmDataTable.create('#leads-table'", $landing);

        $widget = $this->get(route('admin.school-leads.widget'))->assertOk()->getContent();
        $this->assertStringNotContainsString('persistPageLength: true', $widget);
        $this->assertStringNotContainsString("KidsCrmDataTable.create('#leads-table'", $widget);
    }
    ...
    public function test_guest_is_denied_on_index_and_page_length_endpoints(): void
    {
        ...
        $index = $this->get(route('admin.school-leads'));
        ...
        $post = $this->from(route('admin.school-leads'))
            ->post(route('admin.school-leads.columns-settings.save'), [
                'page_length' => 20,
            ]);
    ...
    private function leadsTableCreateChunk(string $html): string
    {
        $pos = strpos($html, "KidsCrmDataTable.create('#leads-table'");
        $this->assertNotFalse($pos, 'KidsCrmDataTable.create(#leads-table) не найден');
