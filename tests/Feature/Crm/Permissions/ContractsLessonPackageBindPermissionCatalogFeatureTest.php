...
/**
 * contracts.lessonPackage.bind: скрытое, не в базовых ролях, выбор абонемента при создании договора.
 */
final class ContractsLessonPackageBindPermissionCatalogFeatureTest extends CrmTestCase
{
    private const PERMISSION = 'contracts.lessonPackage.bind';

    private const DESCRIPTION = 'Договоры: выбор абонемента при создании';

    public function test_permission_exists_hidden_in_contracts_group(): void
    {
        $groupId = (int) DB::table('permission_groups')->where('slug', 'contracts')->value('id');
        ...
        $this->assertSame(55, (int) $row->sort_order);
    }
    ...
    public function test_add_migration_creates_hidden_permission_without_role_backfill(): void
    {
        $src = (string) file_get_contents(base_path(
            'database/migrations/2026_09_19_220000_add_contracts_lesson_package_bind_permission.php'
        ));

        $this->assertStringContainsString("'is_visible'          => 0", $src);
        $this->assertStringContainsString("'name'                => self::PERMISSION_NAME", $src);
        $this->assertStringContainsString("'slug', 'contracts'", $src);
