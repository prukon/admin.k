...
class ContractsDocumentsSectionUiFeatureTest extends ContractsFeatureTestCase
{
    ...
    public function contracts_index_renders_tabs_and_new_toolbar(): void
    {
        $this->get(route('contracts.index'))
            ...
            ->assertSee('<th>Номер договора</th>', false)
            ->assertSee('data-column-key="contract_number"', false)
            ->assertSee('for="colContractNumber">Номер договора</label>', false)
            ->assertSee("key: 'contract_number'", false)
            ...
            ->assertDontSee('data-column-key="fill_expires_at"', false)
            ->assertDontSee('for="colFillExpiresAt">Срок подписания</label>', false)
            ->assertDontSee('<th>Срок подписания</th>', false)
