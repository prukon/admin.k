...
/**
 * P1: доступ к GET /cabinet/system-monitors/online-users и видимости оверлея.
 *
 * @see \Tests\Feature\Crm\Teams\TeamControllerTest::test_store_non_ajax_redirects_and_creates_team
 */
final class SystemMonitorsOnlineUsersAccessFeatureTest extends SystemMonitorsTestCase
{
    public function test_guest_cannot_read_online_users_via_json_or_html(): void
    {
        Auth::logout();
    ...
    public function test_guest_mutating_online_users_is_not_empty_200(): void
    {
        Auth::logout();

        foreach (['POST', 'PATCH', 'PUT', 'DELETE'] as $method) {
            $json = $this->json($method, $this->onlineUsersUrl());
            ...
            $html = $this->call($method, $this->onlineUsersUrl());
    ...
    public function test_admin_without_permission_gets_403_on_json_and_html(): void
    {
        ...
        $html = $this->actingAs($this->user)->get($this->onlineUsersUrl());
        $this->assertNotSame(500, $html->getStatusCode());
        $this->assertNotSame(200, $html->getStatusCode());
        $html->assertForbidden();
    }

    public function test_admin_with_hidden_permission_can_read_online_users_and_sees_overlay(): void
    {
        $this->asAdmin();
    ...
    public function test_trainer_with_granted_permission_can_read_online_users(): void
    {
