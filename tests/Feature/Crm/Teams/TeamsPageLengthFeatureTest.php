...
/**
 * Персональное «Показать N» на /admin/teams: page_length в user_table_settings.
 */
final class TeamsPageLengthFeatureTest extends CrmTestCase
{
    ...
    public function test_index_uses_default_page_length_when_nothing_saved(): void
    {
        ...
        $html = $this->get(route('admin.team.index'))
            ->assertOk()
            ->assertViewHas('teamsPageLength', 10)
            ->getContent();
    ...
    public function test_ajax_save_page_length_persists_without_columns(): void
    {
        ...
        $payload = $this->getJson($this->settingsUrl)
            ->assertOk()
            ->assertExactJson([])
            ->json();
    ...
    public function test_ajax_save_page_length_does_not_wipe_columns(): void
    {
        UserTableSetting::updateOrCreate(
            ['user_id' => $this->user->id, 'table_key' => 'teams_index'],
            ['columns' => ['title' => false, 'order_by' => true]]
        );
    ...
    public function test_guest_cannot_save_page_length(): void
    {
        ...
        $response = $this->postJson($this->settingsUrl, [
            'page_length' => 20,
        ]);
    ...
    public function test_after_changing_show_by_reopening_page_keeps_saved_length(): void
    {
        ...
        $chunk = $this->teamsTableCreateChunk($html);
    ...
    public function test_hiding_columns_then_reopening_page_does_not_reset_show_by(): void
    {
        ...
        $html = $this->get(route('admin.team.index'))
            ->assertOk()
            ->assertViewHas('teamsPageLength', 100)
            ->getContent();

        $this->assertMatchesRegularExpression('/pageLength:\s*100\b/', $this->teamsTableCreateChunk($html));

        $payload = $this->getJson($this->settingsUrl)
            ->assertOk()
            ->assertJsonPath('title', false)
            ->json();
        $this->assertArrayNotHasKey('page_length', $payload);
    }
    ...
    {
        UserTableSetting::updateOrCreate(
            ['user_id' => $this->foreignUser->id, 'table_key' => 'teams_index'],
            ['page_length' => 100]
        );
        ...
        $this->assertMatchesRegularExpression('/pageLength:\s*10\b/', $this->teamsTableCreateChunk($html));
    }
    ...
    public function test_guest_is_denied_on_index_and_page_length_endpoints(): void
    {
        ...
        $index = $this->get(route('admin.team.index'));
    ...
    private function teamsTableCreateChunk(string $html): string
    {
        $pos = strpos($html, "KidsCrmDataTable.create('#teams-table'");
        $this->assertNotFalse($pos, 'KidsCrmDataTable.create(#teams-table) не найден');
